...
if(!class_exists('CommunityOpportunity'))
{

    class CommunityOpportunity
    {
        const POST_TYPE = "hcc_community_opp";
		...
		public function __construct()
		{
		    ...
		    add_action('save_post_hcc_community_opp', array(&$this, 'save_community_opp'));
		...
		public function init()
		{
			...
			register_post_type('hcc_community_opp',
			    array(
			      'labels' => array(
			        'name' => __( 'Community Serving Opportunity' ),
			        'singular_name' => __( 'Serving Opportunity' )
			    ),
			    'description'=>'These are the serving opportunities in the community that Hope is sponsoring/supporting.',
			    'public' => false,
			    'has_archive' => false,
			    'show_ui'=>true,
			    'supports'=>array(
			...
			register_taxonomy('hcc_community_ministry','hcc_community_opp',			
				array(
					'name'=>'Community Ministry',
					'description'=>'These are the different ministries that community opportunities could be a part of.',
					'show_ui'=>true,
					'hierarchical'=>true,
		...
		{           
		    // Add metaboxes		    
		    add_action('add_meta_boxes_hcc_community_opp', array(&$this, 'add_meta_boxes'));
		...
		public function add_meta_boxes()
		{
		    ...
		    add_meta_box(
		    	'hcc_community_serving_opp_ministry',
		    	'Community Ministry',
		    ...
		    add_meta_box(
		    	'hcc_community_serving_opp_campus',
		    	'Campus',
		...
		function add_ministry_box() {
			...
			if (taxonomy_exists('hcc_community_ministry')) {

				$ministry = get_post_meta($post->ID, 'hcc_community_ministry');
				...
				$html_string .= '<select name="hcc_community_ministry">'; 
				$html_string .= '<option value="">select...</option>';

				$terms = get_terms('hcc_community_ministry', array(
					'hide_empty'=>false
				));
				...
				$html_string .= wp_nonce_field( basename( __FILE__ ), 'hcc_community_ministry_nonce');
		...
		function add_campus_box() {
			...
			$html_string = '<p>Select the campus that this opportunity applies to. This will determine who gets the contacted at which campus when someone wants to get involved with an opportunity.</p>';
		...
		public function save_community_opp($post_id) {
			...
			if ($_POST['hcc_community_ministry'] == '') {
				$msg_data .= '<p>Please select a ministry this opportunity applies to</p>';
			} else {
				//update the ministry
				update_post_meta($post_id, 'hcc_community_ministry', $_POST['hcc_community_ministry']);				
				wp_set_post_terms($post_id,$_POST['hcc_community_ministry'],'hcc_community_ministry',false);				
			}
			...
			if ($msg_data != '<div class="error">') {
				...
				set_transient('hcc_community_opp_' . $post_id, $msg_data, 80);							
			} 
		}
		...
		public function display_validation_notices() {
			...
			if (get_transient('hcc_community_opp_' . $post->ID)) {	
			    echo get_transient('hcc_community_opp_' . $post->ID);
			    delete_transient('hcc_community_opp_' . $post->ID);				
			}
		}
    } 
} 
